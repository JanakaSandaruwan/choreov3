...
// Convert converts a field map written with the simple schema shorthand into a JSON schema.
...
// buildObjectSchema turns a simple field map into an object schema with properties and required markers.
...
// schemaFromString interprets shorthand expressions like "string | default='v1'" into schema objects.
...
func applyConstraints(schema *extv1.JSONSchemaProps, constraintExpr, schemaType string) (bool, bool, error) {
	...
	setters := map[string]func(string){
		"pattern":     func(value string) { schema.Pattern = value },
		"title":       func(value string) { schema.Title = value },
		"description": func(value string) { schema.Description = value },
		"format":      func(value string) { schema.Format = value },
	}
	...
	case "string":
		...
		if len(value) >= 2 {
			if (value[0] == '"' && value[len(value)-1] == '"') || (value[0] == '\'' && value[len(value)-1] == '\'') {
				...
				parsed, err := strconv.Unquote(unquoted)
				if err == nil {
					return parsed, nil
				}
			}
...
// tokenizeConstraints splits a constraint expression into tokens while respecting quotes and nested brackets.
