...
// Package componentpipeline provides the main rendering pipeline for Component resources.
//
// The pipeline combines ComponentEnvSnapshot and EnvSettings to generate fully resolved
// Kubernetes resource manifests by:
...
import (
	...
	"github.com/openchoreo/openchoreo/internal/crd-renderer/component-pipeline/addon"
	"github.com/openchoreo/openchoreo/internal/crd-renderer/component-pipeline/context"
	"github.com/openchoreo/openchoreo/internal/crd-renderer/component-pipeline/renderer"
...
func (p *Pipeline) Render(input *RenderInput) (*RenderOutput, error) {
	...
	componentContext, err := context.BuildFromSnapshot(
		input.Snapshot,
		input.Settings,
		input.Metadata,
	)
	...
	resources, err := resourceRenderer.RenderResources(
		input.Snapshot.Spec.ComponentTypeDefinition.Spec.Resources,
		componentContext,
	)
	if err != nil {
	...
	// Get addon definitions and instances from snapshot
	addonMap := make(map[string]*v1alpha1.Addon)
	for i := range input.Snapshot.Spec.Addons {
		addon := &input.Snapshot.Spec.Addons[i]
		addonMap[addon.Name] = addon
	}

	// Process each addon instance from the component
	for _, addonInstance := range input.Snapshot.Spec.Component.Spec.Addons {
		addon, ok := addonMap[addonInstance.Name]
		if !ok {
			metadata.Warnings = append(metadata.Warnings,
				fmt.Sprintf("addon %s referenced but not found in snapshot", addonInstance.Name))
			continue
		}

		// Build addon context
		addonContext, err := context.BuildAddonFromSnapshot(
			input.Snapshot,
			addon,
			addonInstance,
			input.Settings,
		...
		if err != nil {
			return nil, fmt.Errorf("failed to build addon context for %s/%s: %w",
				addonInstance.Name, addonInstance.InstanceID, err)
		}
	...
	if p.options.EnableValidation {
		if err := p.validateResources(resources); err != nil {
			...
			metadata.Warnings = append(metadata.Warnings, fmt.Sprintf("validation warning: %v", err))
		}
	}
...
func (p *Pipeline) postProcessResources(resources []map[string]any, input *RenderInput) error {
	...
	if input.Snapshot != nil {
		commonLabels["openchoreo.org/component"] = input.Snapshot.Spec.Component.Name
		commonLabels["openchoreo.org/environment"] = input.Snapshot.Spec.Environment
	}
...
func (p *Pipeline) validateResources(resources []map[string]any) error {
	for i, resource := range resources {
		...
		if _, ok := resource["apiVersion"].(string); !ok {
		...
		metadata, ok := resource["metadata"].(map[string]any)
		if !ok {
			return fmt.Errorf("resource #%d missing metadata", i)
		}
