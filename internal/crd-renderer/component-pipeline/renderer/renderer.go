...
func (r *Renderer) renderWithForEach(
	...
	for i, item := range items {
		...
		itemContext := cloneContext(context)
...
// This is sufficient since we only add new top-level keys (forEach variable).
func cloneContext(context map[string]any) map[string]any {
	cloned := make(map[string]any, len(context)+1)
	maps.Copy(cloned, context)
	return cloned
}
